...
const create = (req, res) => {
   const datas = req.body
        if(datas.name && datas.country && datas.music && datas.age && datas.hobbie){
            const response = createUser(datas)
...
const update = (req, res) => {
    const id = Number(req.params.id)
    const data = req.body
    if (data.name && data.age && data.music && data.country && data.hobbie) {
